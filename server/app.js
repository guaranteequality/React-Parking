...
store.conventions.findCollectionNameForObjectLiteral = (doc) => {
...
app.use('/getParkingArea', async function(req, res) {
  ...
  const query = await session.query({ collection: "ParkingArea" })
    .all()
...
app.use('/getBookList', async function(req, res) {
  ...
  const query = await session.query({ collection: "Pay" })
    .all()
